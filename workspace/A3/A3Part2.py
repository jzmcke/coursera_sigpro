...
def optimalZeropad(x, fs, f):
    ...
    N_periods = int(math.ceil(1.0*M/T)) # We need to achieve N_periods worth of samples

    N_tot_samples = N_periods*T # Total number of samples

    x_padded = np.zeros(N_tot_samples,dtype=complex)
    ...
    return 20*np.log10(np.abs(X[:hN2+1]))
