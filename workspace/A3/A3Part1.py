...
def minimizeEnergySpreadDFT(x, fs, f1, f2):
    ...
    ns_lcm = ns_f1*ns_f2/gcd(ns_f1,ns_f2)

    N = int(ns_lcm) # select the lowest common multiple of the sinusoid period

    X = fft(x[:N])

    # N = 100
    # split into 0->49 and 50->99; h1n = 50, h2n = 50

    # 101
    # split into 0->50 and 51->100; h1n = 51, h2n = 50

    hN1 = int(math.floor((N+1)/2))
    hN2 = int(math.floor(N/2))

    fft_buffer = np.array(np.zeros(N),dtype=complex)

    fft_buffer[:hN1] = X[hN2:]
    fft_buffer[N-hN2:] = X[:hN2]

    return np.abs(X[:hN2+1])
